...
Model
-----
  Goals_home ~ Poisson(alpha_h * beta_a * gamma)   [non-neutral venues]
...
The fit() method accepts an optional 'weight' column (for time-decay) and
an optional 'neutral' column (True = no home advantage applied).
...
def _neg_log_likelihood(params: np.ndarray,
                        ...
                        weights: np.ndarray, neutral: np.ndarray,
                        n_teams: int) -> float:
    ...
    ha = np.where(neutral, 0.0, log_home)

    lam_h = np.exp(log_att[home] + log_def[away] + ha)
    lam_a = np.exp(log_att[away] + log_def[home])

    nll = -(
        np.dot(weights, poisson.logpmf(home_goals, lam_h)) +
        np.dot(weights, poisson.logpmf(away_goals, lam_a))
    )
    return float(nll)
...
class DixonColesModel:
    ...
    n_matches_      : pd.Series  — raw match count per team
    effective_n_    : pd.Series  — sum-of-weights per team (= n_matches when
                                   no weights supplied, < n_matches with decay)
    """
    ...
    def __init__(self) -> None:
        ...
        self.effective_n_: pd.Series | None = None
        self._fitted = False
    ...
    def fit(self, matches: pd.DataFrame) -> "DixonColesModel":
        ...
        matches : DataFrame with columns
                  ...
                  Optional columns:
                    weight  — per-match weight (float, e.g. time-decay)
                    neutral — True if played at neutral venue (no home adv)
        """
        ...
        weights = (df["weight"].values.astype(float)
                   if "weight" in df.columns else np.ones(len(df)))
        neutral = (df["neutral"].astype(bool).values
                   if "neutral" in df.columns else np.zeros(len(df), dtype=bool))
        ...
        # raw match counts and weighted counts per team (vectorised)
        home_teams = df["home_team"].values
        away_teams = df["away_team"].values
        counts = (pd.Series(home_teams).value_counts()
                  .add(pd.Series(away_teams).value_counts(), fill_value=0)
                  .reindex(all_teams, fill_value=0).astype(int))
        eff_h = pd.Series(weights, index=home_teams).groupby(level=0).sum()
        eff_a = pd.Series(weights, index=away_teams).groupby(level=0).sum()
        eff   = eff_h.add(eff_a, fill_value=0).reindex(all_teams, fill_value=0.0)
        self.n_matches_   = counts
        self.effective_n_ = eff
        ...
        res = minimize(
            _neg_log_likelihood,
            x0,
            args=(home, away, hg, ag, weights, neutral, n),
            method="L-BFGS-B",
